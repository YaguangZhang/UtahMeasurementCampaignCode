...
%
% Yaguang Zhang, Purdue, 08/19/2021
...
for idxRxGain = 1:numDatasets
    ...
    curExpFoldername = ['USRP_' num2str(curRxGain), 'dB'];
...
for idxDataset = 1:numDatasets
    ...
    if FLAG_USE_FILTERED_OUTPUT_FILES
        ...
        curCalDataLogs = rdir(curDatasetAbsPath, ...
            'regexp(name, ''(_filtered\.log$)'')');
    else
        % Use the original output files.
        curCalDataLogs = rdir(curDatasetAbsPath, ...
            'regexp(name, ''(_samples\.log$)'')');
    end
    % Make sure the log files are sorted by name before loading. Example
    % file name: u0_a8_samples.log (USRP gain = 0dB; attenuator value =
    % 8dB).
    [~, sortedIs] = sort( str2double(cellfun(@(ts) ts{1} , ...
        regexp( {curCalDataLogs.name}, ...
        '_a(\d+)_', 'tokens') ...
        )));
    ...
    for idxCurMeas = 1:curNumMeas
        ...
        curCalData{idxCurMeas} = curSignal;
    end
...
for idxDataset = 1:numDatasets
    ...
    for idxCurMeas = 1:curNumMeas
        ...
        % Fetch the samples for calibration.
